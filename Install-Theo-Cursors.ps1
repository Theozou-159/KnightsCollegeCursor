...
if ($Apply) {
  ...
  try {
    ...
    Write-Warning "The Theo cursor files were applied, but Windows did not allow the scheme name to be set automatically."
  }
...
if ($Apply) {
  ...
  Write-Host "Theo Cursor has been installed." -ForegroundColor Green
  Write-Host "Your current cursor scheme was not changed." -ForegroundColor Yellow
  Write-Host "To switch later: Mouse Properties -> Pointers -> Scheme -> Theo Cursor."
  try {
